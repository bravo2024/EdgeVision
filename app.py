...
def try_load_cached_results():
    ...
    if _TORCH_AVAILABLE:
        for name, pth_path in model_files.items():
            if pth_path.exists():
                try:
                    st.session_state.models[name] = torch.load(pth_path, map_location="cpu")
                except Exception:
...
with tab_bench:
    ...
    if st.session_state.benchmarks:
        ...
        **Reading the results:**
        ...
        - **MobileNetV2** (2.3M params) trades a few accuracy points for a ~5× parameter
          reduction versus ResNet18, via depthwise separable convolutions
        - **ShuffleNetV2** (1.3M params) is the smallest of the three; channel shuffle keeps
          information flowing between grouped convolutions
        - **ResNet18** (11.7M params) is the accuracy ceiling here, but its size and latency
          make it the least attractive for constrained devices

        **Why depthwise separable convolutions are cheaper:**
        ```
...
with st.expander("Deployment & production notes"):
    ...
    **EdgeVision** — Edge deployment guide:
    ...
    Typical runtimes per platform: TensorRT on NVIDIA Jetson, Core ML on iOS/macOS,
    TFLite/NNAPI on Android, plain ONNX Runtime on Raspberry Pi and generic ARM boards.
    """)
