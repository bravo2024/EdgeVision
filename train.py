...
def main():
    ...
    from src.data import load_cifar10, make_synthetic_cifar, make_torch_loaders
    ...
    # Load data
    if args.synthetic:
        data = make_synthetic_cifar(seed=args.seed)
        print(f"Using synthetic data: {data['n_train']} train, {data['n_test']} test")
    else:
        data = load_cifar10()
    ...
    print(f"\nSaved weights -> {weights_path}, metrics -> models/metrics.json")
